...
Citizen.CreateThread(function()
	...
	if char > 8 then
		print(('[mil_vehicleshop] [^3WARNING^7] Plate character count reached, %s/8 characters!'):format(char))
	end
end)
...
MySQL.ready(function()
	MySQL.query('SELECT * FROM mil_vehicle_categories', {}, function(_categories)
		...
		MySQL.query('SELECT * FROM mil_vehicles', {}, function(_vehicles)
			...
			TriggerClientEvent('mil_vehicleshop:sendCategories', -1, categories)
			TriggerClientEvent('mil_vehicleshop:sendVehicles', -1, vehicles)
		end)
	end)
end)
...
RegisterNetEvent('mil_vehicleshop:setVehicleOwnedPlayerId', function(playerId, vehicleProps, model, label)
			...
			if result[1] then
				...
				MySQL.query('DELETE FROM cardealer_vehicles WHERE id = @id', {
					...
					if rowsChanged == 1 then
						...
						MySQL.insert('INSERT INTO vehicle_sold (client, model, plate, soldby, date) VALUES (@client, @model, @plate, @soldby, @date)', {
...
ESX.RegisterServerCallback('mil_vehicleshop:getSoldVehicles', function(source, cb)
	MySQL.query('SELECT client, model, plate, soldby, date FROM vehicle_sold', {}, function(result)
		cb(result)
	end)
end)

RegisterNetEvent('mil_vehicleshop:rentVehicle', function(vehicle, plate, rentPrice, playerId)
	local xPlayer, xTarget = ESX.GetPlayerFromId(source), ESX.GetPlayerFromId(playerId)
		...
		MySQL.query('SELECT id, price FROM cardealer_vehicles WHERE vehicle = @vehicle LIMIT 1', {
			...
			if result[1] then
				...
				MySQL.query('DELETE FROM cardealer_vehicles WHERE id = @id', {
					...
					if rowsChanged == 1 then
						MySQL.insert('INSERT INTO rented_vehicles (vehicle, plate, player_name, base_price, rent_price, owner) VALUES (@vehicle, @plate, @player_name, @base_price, @rent_price, @owner)', {
							['@vehicle'] = vehicle,
							['@plate'] = plate,
...
RegisterNetEvent('mil_vehicleshop:getStockItem', function(itemName, count)
...
RegisterNetEvent('mil_vehicleshop:putStockItems', function(itemName, count)
	local _source = source
	local xPlayer = ESX.GetPlayerFromId(_source)
...
ESX.RegisterServerCallback('mil_vehicleshop:getCategories', function(source, cb)
	cb(categories)
end)

ESX.RegisterServerCallback('mil_vehicleshop:getVehicles', function(source, cb)
	cb(vehicles)
end)

ESX.RegisterServerCallback('mil_vehicleshop:buyVehicle', function(source, cb, model, plate)
...
print('\n\n===========================================\n^0[^3mil_vehicleshop^0] ^2MIL_Scripts#1739^0\n===========================================\n')

ESX.RegisterServerCallback('mil_vehicleshop:getCommercialVehicles', function(source, cb)
	MySQL.query('SELECT price, vehicle FROM cardealer_vehicles ORDER BY vehicle ASC', {}, function(result)
		cb(result)
	end)
end)

ESX.RegisterServerCallback('mil_vehicleshop:buyCarDealerVehicle', function(source, cb, model)
	local xPlayer = ESX.GetPlayerFromId(source)
		...
		if modelPrice then
			TriggerEvent('esx_addonaccount:getSharedAccount', 'society_cardealer', function(account)
				if modelPrice and xPlayer.getAccount('coinlegend') >= modelPrice then
					...
					MySQL.insert('INSERT INTO cardealer_vehicles (vehicle, price) VALUES (@vehicle, @price)', {
						['@vehicle'] = model,
						['@price'] = modelPrice
					}, function(rowsChanged)
						cb(true)
...
RegisterNetEvent('mil_vehicleshop:returnProvider', function(vehicleModel)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if xPlayer.job.name == 'cardealer' then
		MySQL.query('SELECT id, price FROM cardealer_vehicles WHERE vehicle = @vehicle LIMIT 1', {
			...
			if result[1] then
				...
				print(('[mil_vehicleshop] [^3WARNING^7] %s attempted selling an invalid vehicle!'):format(xPlayer.identifier))
			end
		end)
	end
end)

ESX.RegisterServerCallback('mil_vehicleshop:getRentedVehicles', function(source, cb)
...
ESX.RegisterServerCallback('mil_vehicleshop:giveBackVehicle', function(source, cb, plate)
...
ESX.RegisterServerCallback('mil_vehicleshop:resellVehicle', function(source, cb, plate, model)
	...
	if xPlayer.job.name == 'cardealer' then
		...
		if not resellPrice then
			print(('[mil_vehicleshop] [^3WARNING^7] %s attempted to sell an unknown vehicle!'):format(xPlayer.identifier))
			cb(false)
		else
						...
						if result[1] then -- does the owner match?
							...
							if vehicle.model == model then
								if vehicle.plate == plate then
									...
									print(('[mil_vehicleshop] [^3WARNING^7] %s attempted to sell an vehicle with plate mismatch!'):format(xPlayer.identifier))
									cb(false)
								end
							else
								print(('[mil_vehicleshop] [^3WARNING^7] %s attempted to sell an vehicle with model mismatch!'):format(xPlayer.identifier))
								cb(false)
							end
						end
					end)
...
ESX.RegisterServerCallback('mil_vehicleshop:getStockItems', function(source, cb)
	TriggerEvent('esx_addoninventory:getSharedInventory', 'society_cardealer', function(inventory)
		cb(inventory.items)
	end)
end)

ESX.RegisterServerCallback('mil_vehicleshop:getPlayerInventory', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
...
ESX.RegisterServerCallback('mil_vehicleshop:isPlateTaken', function(source, cb, plate)
...
ESX.RegisterServerCallback('mil_vehicleshop:retrieveJobVehicles', function(source, cb, type)
	local xPlayer = ESX.GetPlayerFromId(source)
...
RegisterNetEvent('mil_vehicleshop:setJobVehicleState', function(plate, state)
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	MySQL.update('UPDATE owned_vehicles SET `stored` = @stored WHERE plate = @plate AND job = @job', {
		...
		if rowsChanged == 0 then
			print(('[mil_vehicleshop] [^3WARNING^7] %s exploited the garage!'):format(xPlayer.identifier))
		end
	end)
end)

function UnrentVehicleAsync(identifier, plate)
	MySQL.query('DELETE FROM rented_vehicles WHERE identifier = @identifier AND plate = @plate', {
...
function PayRent(d, h, m)
	...
	print('[mil_vehicleshop] [^2INFO^7] Paying rent cron job started')
	...
	MySQL.query('SELECT owner, rent_price, plate FROM rented_vehicles', {}, function(result)
		...
		print(('[mil_vehicleshop] [^2INFO^7] Paying rent cron job took %s seconds'):format(elapsedTime))
	end)
end
